...
public:
    employee(char *pName, int eId, char g, char *dName, char *aw, int jy) : personalInfo(pName, eId, g), departmentInfo(dName, aw)
    {
        joining_year = jy;
    }
...
public:
    serverMaintenanceOfficer(char *pName, int eId, char g, char *dName, char *aw, int jy, int phr) : employee(pName, eId, g, dName, aw, jy)
    {
    ...
    void showEmployeeInfo()
    {
        ...
        showPersonalInfo();
        showDepartmentalInfo();
        cout << "Per hour salary: " << per_hour_salary << endl;
    }
};
