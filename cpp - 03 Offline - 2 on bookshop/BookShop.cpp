...
public:
    ...
    BookShop(const BookShop &bs)
    {
        ...
        size = bs.size;
        books = new Book[size];
        this->addBook(bs.books, bs.count);
    }
    ...
    void addBook(Book *ba, int count)
    {
        ...
        for (int i = 0, j = this->count; i < count; i++)
        {
            if (i + j == size)
                break;
            addBook(ba[i]);
    ...
    BookShop mergeShop(BookShop b)
    {
        ...
        int size = this->count + b.count;
        BookShop merged("MergedShop", size);
        merged.addBook(this->books, this->count);
